...
import { service } from '../components/export'; // Ensure `service` is an array of objects
        ...
        <div className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'} pb-20`}>
            <section id="services" className={`${darkMode ? 'dark bg-gray-800' : 'light bg-red-100'} lg:w-[95%] w-full h-fit m-auto rounded-xl flex flex-col justify-center items-start lg:px-20 px-6 py-20 gap-10`}>
                ...
                <div id="service-box" className='grid lg:grid-cols-3 grid-cols-1 justify-center items-center gap-8'>
                    {service && service.map((item, index) => ( // Ensure `service` is defined and mapped correctly
                        <div 
                            data-aos="zoom-in" 
                            data-aos-delay="200" 
                            key={index} 
                            className='bg-white dark:bg-gray-900 h-[350px] px-8 py-16 flex flex-col justify-center items-start gap-4 rounded-xl border-b-[5px] border-red-600 hover:bg-red-300 dark:hover:bg-gray-700 cursor-pointer transition-all duration-300'
                        >
                            <div className='p-6 rounded-full bg-red-200 dark:bg-gray-700'>
                            ...
                            <p className='text-lg text-slate-700 dark:text-gray-300'>{item.desc}</p>
